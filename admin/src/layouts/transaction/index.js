...
function Transactions() {
  ...
  const buildDeposit = (users, index) => {
    ...
    users.forEach((element, index) => {
      ...
      temp.coin = (
        <MDTypography variant="caption" color="text" fontWeight="medium">
          {element && element.asset ? element.asset : element.coin}
        </MDTypography>
      )
      temp.token = (
        <MDTypography variant="caption" color="text" fontWeight="medium">
  ...
  const getdata1 = async (type) => {
    setLoading(true);
    const url = `${endpoints.overalltransaction}?type=${type}`;
    try {
      const data = await path.getCall({ url });
      const result = await data.json();
      if (result.status === true) {
          ...
          <Grid item xs={12} md={7} >
            <Box>
              <TabList onChange={handleChange} aria-label="lab API tabs example">
                ...
                <Tab label="Debit" value="3" />
                <Tab label="Deposit History" value="4" />
                <Tab label="Withdraw History" value="5" />
              </TabList>
            </Box>
          </Grid>
        </Grid>
