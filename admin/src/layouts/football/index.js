...
function Users() {
  ...
  const onFormsubmit = async (values) => {
    ...
    const url = endpoints.editBetAmount;
    try {
  ...
  const getdata = async () => {
    setLoading(true);
    const url = endpoints.footBallBtAmtMatch;
    try {
      const data = await path.getCall({ url });
      const result = await data.json();
      if (result.status === true) {
  ...
  const buildData = (users, index) => {
    ...
    setCollection({
      columns: [
        ...
        { Header: "Match Time", accessor: "match_time", width: "10%", align: "center" },
        { Header: "Match Status", accessor: "status", width: "10%", align: "center" },
