...
const cricketBetModelAdmin = require('../models/cricketBettingAdmin')
const axios = require('axios')
// const adminWalletModel = require('../models/adminModel')
...
exports.createBetting = async (req, res, next) => {
  try {
    ...
    const tournament = await touranamentModel.findOne({
        ...
        if (getUserBetting?.length === 0) {
          if (findData && findData !== null) {
            ...
            if (findData.betting_limit > findData.betting_count) {
              ...
              const updateBalance = await walletModel.findOneAndUpdate(
                { user_id: userId },
                { $inc: { balance: -parseFloat(userInput.bet_amount) } },
                { new: true },
              )

              const updateAdminBalance = await cricketLiveScore.findOneAndUpdate(
                { match_id: userInput?.match_id },
                { $inc: { totalBetAmount: parseFloat(userInput.bet_amount) } },
                { new: true },
              )
          ...
          return APIRes.getErrorResult('User already bet on this option', res)
        }
      } else {
  ...
  try {
    ...
    const result = resultData.map((item, i) => {
      return {
        ...
        status: item.status,
        createdAt: Date.now(item.createdAt),
...
exports.createFootBallBetting = async (req, res, next) => {
  try {
    ...
    findQuery.user_id = userId
    findQuery.match_id = userInput.match_id
    const tournament = await footBallBtAmtModel.findOne({
      match_id: userInput.match_id,
    })

 if(userInput.is_player_bet==="true"){
      if(!userInput.player_id && !userInput.player_name){
        return APIRes.getErrorResult('Require Player Details', res)
      }
    }
    ...
    if (tournament) {
          ...
          if (!getUserBetting) {
            const updateBalance = await walletModel.findOneAndUpdate(
              { user_id: userId },
              {
                $set: {
                  balance:
                    parseInt(walletData.balance) -
                    parseInt(userInput.bet_amount),
                },
              },
              { new: true },
            )
            ...
            const createTransactionObj = {
              ...
              no_of_token: userInput.bet_amount,
              availablebalance:
                parseInt(walletData.balance) - parseInt(userInput.bet_amount),
            ...
            return APIRes.getErrorResult(
              'User already bet on this option',
              res,
            )
          }
        } else {
      ...
      return APIRes.getErrorResult(
        "You can't bet this match Please contact Admin!",
        res,
      )
    }
  } catch (error) {
...
exports.createCricketBetting = async (req, res, next) => {
  try {
    let errors = validationResult(req)
    if (!errors.isEmpty()) {
      throw errors.array()
    ...
    const checkBet = await axios.get(
      `https://api.thesports.com/v1/cricket/match/detail_live?user=${footballUser}&secret=${footballSecret}`,
    )

   

    const checkBetArray = checkBet?.data?.results?.find(
      (item) => item.id === userInput.match_id
    )


    // return

    const tournament = await cricketBetModelAdmin.findOne({
      match_id: userInput.match_id,
    })

    // console.log(new Date(tournament.match_time*1000).toLocaleString() , Date.now());

    var currentTime = new Date(tournament.match_time*1000); // get the current time
var startTime = new Date(currentTime.getTime() - (5 * 60 * 1000)); // subtract 5 minutes in milliseconds





var matchTime = new Date(currentTime.getTime()); // get the current time

  var betTime = new Date(matchTime.getTime() - (5 * 60 * 1000)); // allow betting 5 minutes before the match
...
try {
  ...
  if (checkBetArray?.timeline?.length === 1) {
    console.log('firstinnings')
  }
  if (checkBetArray?.timeline?.length === 2) {
    const checkArray = checkBetArray?.timeline?.filter((item, i) => {
      if (item.team_id === userInput?.betting_team_key) {
        console.log(item.overs[item.overs.length - 1][0]);
        if (item.overs[item.overs.length - 1][0] >= userInput?.no_of_over) {
          return true
        }
      }
    })
    ...
    if (checkArray?.length !== 0) {
      return APIRes.getErrorResult('This Over is Finished', res)
    }
  }
  ...
  // console.log(tournament,'fgjhjhhh');
  if (tournament) {
    const walletData = await walletModel.findOne({
      user_id: userId,
    })
    ...
    if (walletData) {
      ...
      if (walletData.balance >= userInput.bet_amount) {
        ...
        parseInt(userInput.no_of_ball)
        parseInt(userInput.no_of_over)
        parseInt(userInput.no_of_run)
        parseInt(userInput.betting_team_key)

        const getUserBetting = await bettingModel.findOne(userInput)
        if (!getUserBetting) {
          const updateBalance = await walletModel.findOneAndUpdate(
            { user_id: userId },
            {
          ...
          const findData = await bettingModel.create(userInput)
  ...
  console.log(currentTime.toLocaleString(),matchTime.toLocaleString());

  var currentTimeIST = new Date(currentTime.toLocaleString("en-US", {timeZone: "Asia/Kolkata"}));

  console.log(currentTimeIST.getTime() >= betTime.getTime() && currentTimeIST.getTime() < matchTime.getTime(), currentTimeIST.getTime() >= matchTime.getTime());

  if (currentTimeIST.getTime() >= betTime.getTime() && currentTimeIST.getTime() < matchTime.getTime()) {
    // Call betOnMatch() function if the current time is within 5 minutes before the match time
    // betOnMatch();
    handleBet()
  }
  else if (currentTimeIST.getTime() >= matchTime.getTime()) {
    // console.log('asdsad');
    handleBet()
  } 
  else{
    return APIRes.getErrorResult('Match not started', res)
  }



    // if (checkBetArray === undefined) {
    //   return APIRes.getErrorResult('Match not started', res)
    // }
