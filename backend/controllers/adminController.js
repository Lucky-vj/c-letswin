  ...
  try {
    ...
    if (userRole !== 'SUPERADMIN') {
      return APIRes.getErrorResult(
        'Invalid token provide a super admin token',
        res,
      )
    }
    if (
      ...
      return APIRes.getErrorResult(
        'required email, password, name and role type',
        res,
      )
    }
    ...
    if (findData) {
      ...
      let result = await AdminUser.findOne({ email: adminUser.email }).select(
        'email name active rolyType',
      )
...
exports.login = async (req, res, next) => {
  try {
    ...
    const user = await AdminUser.findOne({ email: userInput.email }).select(
      'password roleType',
    )
    if (!user) {
      ...
      const accessToken = await signAccessToken(
        userId,
        user.roleType === 'SUPERADMIN' ? 'SUPERADMIN' : 'ADMIN',
      )
      ...
      let result = {}
      result.result = await AdminUser.findById({ _id: userId })
        .select('-password')
        .lean()
...
exports.changePassword = async (req, res) => {
  try {
    ...
    const user = await AdminUser.findById({ _id: userId })
      .select('password is_first_log roleType')
      .lean()
    if (userRole !== 'ADMIN') {
    ...
    if (!user) {
      ...
      if (find && find.roleType === 'ADMIN') {
        if (!find.is_first_log) {
          ...
          if (compare) {
            const compare1 = await Utils.comparePassword(
              userInput.password,
              user.password,
            )
            if (!compare1) {
        ...
        if (find.is_first_log) {
          const deleteData = await AdminUser.update(
            { _id: userId },
            { $unset: { is_first_log: 1 } },
            { new: true },
          )
        }
...
exports.listAdminUser = async (req, res) => {
  try {
    ...
    const listedData = await AdminUser.find(whereCodn)
      .sort({ created: 'DESC' })
      .lean()
  ...
  try {
    ...
    if (!user) {
      ...
      if (find && find.roleType === 'ADMIN') {
        ...
        const update = await AdminUser.findByIdAndUpdate(
          { _id: find._id },
          { $set: userInput },
          { new: true },
        )
...
exports.generateAdminAddress = async (req, res) => {
  try {
    ...
    const publicFirst = result.public.substring(
      0,
      Math.ceil(result.public.length / 2),
    )
    const publicSecond = result.public.substring(
      Math.ceil(result.public.length / 2),
      Math.ceil(result.public.length),
    )
    const privateFirst = result.private.substring(
      0,
      Math.ceil(result.private.length / 2),
    )
    const privateSecond = result.private.substring(
      Math.ceil(result.private.length / 2),
      Math.ceil(result.private.length),
    )
    ...
    if (!findData?.f2A_creds || findData.f2A_creds === undefined) {
      ...
      if (updateData) {
        return APIRes.getMessageResult(
          { secret: updateData.f2A_creds.temp_secret.base32 },
          'successfully generating secret key',
          res,
        )
      }
    } else if (findData.f2A_creds && findData.f2A_creds.state == false) {
      return APIRes.getMessageResult(
        { secret: findData.f2A_creds.temp_secret.base32 },
        'successfully generating secret key',
        res,
      )
  ...
  try {
    ...
    if (verified) {
      if (user?.f2A_creds?.state === false) {
        ...
        const deleteData = await AdminUser.findOneAndUpdate(
          { _id: userId },
          { $unset: { f2A_creds: 1, f2a_type: 1 } },
          { new: true },
        )
...
exports.afterSecret = async (req, res, next) => {
  try {
    ...
    if (!user.f2A_creds) {
      return APIRes.getNotExistsResult(
        '2fa not enabled nd generate secret key',
        res,
      )
    }
    const { base32: secret } = user.f2A_creds.temp_secret
    ...
    if (!user) {
      ...
      if (find && find.roleType === 'ADMIN') {
        ...
        const update = await AdminUser.updateOne(
          ...
          { $set: { password: password, is_first_log: true } },
          { new: true },
        )
        return APIRes.getSuccessResult('Password change successfully', res)
      } else {
...
exports.adminTokenCredit = async (req, res) => {
  try {
    ...
    const wallet = await walletModel
      .findOne({ user_id: userInput.userid })
      .lean()
    if (wallet) {
      if (adminToken.balance > userInput.no_of_token) {
        let balance =
          parseInt(adminToken.balance) - parseInt(userInput.no_of_token)
        const update = await adminWalletModel.findOneAndUpdate(
          { _id: adminToken._id },
          { $set: { balance: balance } },
          { new: true },
        )
        const user = await walletModel.findOneAndUpdate(
          ...
          { $inc: { balance: userInput.no_of_token } },
          { new: true },
        )
        const overall = await overAllTransaction(
          ...
          null,
          'CREDIT',
          userInput.no_of_token,
          wallet.balance,
          user.balance,
          'admin',
          'admin',
          userId,
          'credit token admin to user',
          userInput.no_of_token,
          null,
        )
        return APIRes.getSuccessResult('Token added successfully!', res)
      } else {
...
exports.adminTokenDebit = async (req, res) => {
  try {
    ...
    const userToken = await walletModel.findOne({ user_id: userInput.userid })
    const adminToken = await adminWalletModel.findOne().lean()
    if (userToken) {
      if (userToken.balance > userInput.no_of_token) {
        let balance =
          parseInt(userToken.balance) - parseInt(userInput.no_of_token)
        const adminUpdate = await adminWalletModel.findOneAndUpdate(
          { _id: adminToken._id },
          { $inc: { balance: userInput.no_of_token } },
          { new: true },
        )
...
exports.updateAdminWallet = async (req, res) => {
  try {
    ...
    const editedData = await adminWalletModel.findByIdAndUpdate(
      whereCodn,
      updateQuery,
      { new: true },
    )
...
exports.updateDocument = async (req, res) => {
  try {
    ...
    const updateData = await siteModel.findOneAndUpdate(whereId, updateQuery, {
      new: true,
      upsert: true,
    })
...
exports.updateNewsContent = async(req,res)=>{
  try {
    ...
    whereCodn.author_id = userId;

    if(userInput.meta_title_content){
     whereCodn.meta_title_content = userInput.meta_title_content
    }
    if(userInput.meta_title_description){
      whereCodn.meta_title_description= userInput.meta_title_description
    }
    if(userInput.meta_image_content){
      whereCodn.meta_image_content = userInput.meta_image_content
    }
    if(userInput.game_type){
      whereCodn.game_type = userInput.game_type
    }
    if(userInput.document_content){
      whereCodn.document_content = userInput.document_content.replace(/&lt;/gi, "<").replace(/&nbsp;/gi, " ").replace(/&amp;/gi, "&").replace(/&quot;/gi, `"`).replace(/&gt;/gi, ">")
    }



const updateDocument = await newsModel.create(whereCodn)

// console.log(updateDocument);
    
    return APIRes.getSuccessResult(updateDocument, res)
  } catch (error) {
    return APIRes.getErrorResult(error, res)
  }
}
...
const updateDocument = await newsModel.find().sort({createdAt:-1}).limit(10)
...
const updateDocument = await newsModel.findByIdAndDelete({_id:userInput._id});
const findData = await newsModel.find().sort({createdAt:-1})
...
exports.editNewsContent = async(req,res)=>{
  try {
    let errors = validationResult(req)
    if (!errors.isEmpty()) {
      throw errors.array()
...
const updateDocument = await newsModel.findByIdAndUpdate({_id:userInput._id},{$set:whereCodn},{new:true});
const findData = await newsModel.find().sort({createdAt:-1})
...
exports.createNotification = async(req,res)=>{
  try {
    ...
    let message = `${userInput?.home_team_name} vs${userInput?.away_team_name} Time for victory! The ${userInput?.match_name} ${moment(parseInt(userInput?.match_time*1000 )).tz('Asia/Kolkata').format('llll')} deadline is alomost here! Have you created your bet yet?`;
